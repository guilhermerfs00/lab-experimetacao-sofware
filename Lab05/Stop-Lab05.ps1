<#
.SYNOPSIS
    Para o dashboard Lab05 (Streamlit) e o Cloudflare Tunnel.

.DESCRIPTION
    Lê os PIDs salvos por Start-Lab05.ps1 e encerra os processos
...
if (-not (Test-Path $PidFile)) {
    Write-Warning "Arquivo .pids não encontrado. Lab05 não parece estar em execução."
    exit 0
}
...
if ($pids.StreamlitPid) {
    ...
    if ($proc) {
        ...
        Write-Host ">> Streamlit (PID $($pids.StreamlitPid)) já não estava em execução." -ForegroundColor Yellow
    }
}
...
if ($pids.CloudflaredPid) {
    ...
    if ($proc) {
        ...
        Write-Host ">> cloudflared (PID $($pids.CloudflaredPid)) já não estava em execução." -ForegroundColor Yellow
    }
}
...
Write-Host ""
Write-Host ">> Lab05 encerrado com sucesso." -ForegroundColor Green
